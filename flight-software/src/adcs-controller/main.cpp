...
float rampRateCmdX = 0.0, rampRateCmdY = 0.0, rampRateCmdZ = 0.0;  // rad/s^2
...
void printWheelStatus(const char* name, BLDCMotor& motor, MagneticSensorSPI& sensor,
                      float speedCmd, float torqueCmd, float rampRateCmd) {
  Serial.print(name);
  Serial.print(" | τ_cmd: ");      Serial.print(torqueCmd, 6);
  Serial.print(" N*m  ramp: ");    Serial.print(rampRateCmd, 3);
  Serial.print(" rad/s^2  v_cmd: "); Serial.print(speedCmd, 3);
...
void updateWheel(BLDCMotor& motor, float& speedCmd, float torqueCmd, float rampRateCmd,
  ...
  float accelCmd;
  if (rampRateCmd != 0.0f) {
    // ramp-rate command: use rad/s^2 directly
    accelCmd = clampFloat(rampRateCmd, -MAX_WHEEL_ACCEL, MAX_WHEEL_ACCEL);
  } else {
    accelCmd = clampFloat(torqueCmd / WHEEL_INERTIA, -MAX_WHEEL_ACCEL, MAX_WHEEL_ACCEL);
  }
...
void dispatchMotor(BLDCMotor& motor, MagneticSensorSPI& sensor,
                   float& speedCmd, float& torqueCmd, float& rampRateCmd,
                   uint32_t& lastUpdate, const char* name, char* cmd) {
  if (!cmd || cmd[0] == '\0') {
    printWheelStatus(name, motor, sensor, speedCmd, torqueCmd, rampRateCmd);
    return;
  }
  ...
  switch (sub) {
    ...
    case 'R':
      torqueCmd   = 0.0;
      rampRateCmd = clampFloat(atof(arg), -MAX_WHEEL_ACCEL, MAX_WHEEL_ACCEL);
      lastUpdate  = micros();
      Serial.print(name); Serial.print(" ramp rate: "); Serial.print(rampRateCmd, 3); Serial.println(" rad/s^2");
      break;
    case 'B':
      torqueCmd    = 0.0;
      rampRateCmd  = 0.0;
    ...
    default:
      ...
      Serial.println("'  valid: T V R B S");
  }
}
...
void doHelp(char* cmd) {
  ...
  Serial.println(F("  <axis>R<val>   ramp rate (rad/s^2)  e.g.  XR50      ZR-100"));
...
void runProtoCommands() {
  while (Serial2.available()) {
    ...
    if (c == '\n') {
      ...
      protoBufIdx = 0;  // overflow — discard and reset
    }
  }
}
...
void setup() {
  Serial.begin(115200);
  Serial2.begin(115200, SERIAL_8N1, PROTO_RX, PROTO_TX);
  delay(1500);

  //XV10Serial.println(F("\n=== THREE-WHEEL REACTION WHEEL CONTROL ==="));
